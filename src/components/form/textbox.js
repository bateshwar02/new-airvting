...
function create(overrides = {}) {
  ...
  textbox.getConfig = overrides.getConfig
        || function getConfig(locals) {
          return new TextboxConfig(locals.config || {});
        };

  textbox.getAttrs = overrides.getAttrs
        || function getAttrs(obj) {
          ...
          if (locals.type !== 'file') {
            attrs.value = locals.value || locals.attrs.value;
          }
          attrs.onChange = onChange(locals, reRender, attrs);

          attrs.id = `${locals.config.idPrefix || ''}${attrs.id}`;
  ...
  textbox.renderHiddenTextbox = overrides.renderHiddenTextbox
        || function renderHiddenTextbox(locals) {
          return <input type="hidden" value={locals.value} name={locals.attrs.name} />;
        };

  textbox.renderStatic = overrides.renderStatic
        || function renderStatic(locals) {
          return <p className="form-control-static">{locals.value}</p>;
        };

  textbox.renderTextbox = overrides.renderTextbox
        || function renderTextbox(locals) {
          if (locals.type === 'static') {
            return textbox.renderStatic(locals);
          }
          return locals.type !== 'textarea' ? textbox.renderInputGroup(locals) : getInputGroup(textbox.renderTextarea(locals), locals);
        };

  textbox.renderInputGroup = overrides.renderInputGroup
  ...
  textbox.renderInput = overrides.renderInput
  ...
  textbox.renderTextarea = overrides.renderTextarea
  ...
  textbox.renderLabel = overrides.renderLabel
  ...
  textbox.renderError = overrides.renderError
        || function renderError(locals) {
          return getError(locals);
        };

  textbox.renderHelp = overrides.renderHelp
        || function renderHelp(locals) {
          return getHelp(locals);
        };

  textbox.renderVertical = overrides.renderVertical
  ...
  textbox.renderHorizontal = overrides.renderHorizontal
