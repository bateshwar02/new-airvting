...
const getStyles = {
  ...
  option: (provided, state) => ({
    ...provided,
    backgroundColor: state.isSelected ? '#f4f6fc' : 'transparent',
...
function create(overrides = {}) {
  ...
  select.renderSelectbox = overrides.renderSelectbox
        || function renderSelectbox(obj) {
          ...
          localsObj.id = `${locals.config.idPrefix || ''}${locals.attrs.name}-${locals.attrs.id}`;
          localsObj.inputId = `${locals.config.idPrefix || ''}${locals.attrs.name}-input-${locals.attrs.id}`;
  ...
  select.renderLabel = overrides.renderLabel
  ...
  select.renderError = overrides.renderError
        || function renderError(locals) {
          return getError(locals);
        };

  select.renderHelp = overrides.renderHelp
        || function renderHelp(locals) {
          return getHelp(locals);
        };

  select.renderVertical = overrides.renderVertical
  ...
  select.renderHorizontal = overrides.renderHorizontal
