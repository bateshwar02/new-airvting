...
function create(overrides = {}) {
  ...
  date.getConfig = overrides.getConfig
        || function getConfig(locals) {
          return new SelectboxConfig(locals.config || {});
        };

  date.renderDatebox = overrides.renderDatebox
        || function renderDatebox(dateLocal) {
          ...
          const newLocals = {
            onChange: (val) => {
              locals.onChange(val.getTime());
              if (Utils.isFunction(reRender)) {
                reRender(val.getTime());
              }
            },
  ...
  date.renderLabel = overrides.renderLabel
  ...
  date.renderError = overrides.renderError
        || function renderError(locals) {
          return getError(locals);
        };

  date.renderHelp = overrides.renderHelp
        || function renderHelp(locals) {
          return getHelp(locals);
        };

  date.renderVertical = overrides.renderVertical
  ...
  date.renderHorizontal = overrides.renderHorizontal
